    ...
    internal class AudioTrack {
        ...
        private int NextClipIdx {
            get {
                if (_nextClipIdx < 0) {
                    ...
                    if (numClips > 0) {
                        if (++trackQueuePos >= numClips || trackQueue == null || trackQueuePos >= trackQueue.Length) { //we hit the end so we need to reset queue
                            ...
                            trackQueuePos = 0;
                            //make sure we arn't playing the same track we just finished
                            if (numClips > 1 && trackQueue != null && trackQueue[trackQueue.Length - 1] == playlist[0]) {
                                playlist.RemoveAt(0);
                                playlist.Insert(Helpers.GetRandom(1, numClips - 1), trackQueue[trackQueue.Length - 1]);
                            }
        ...
        public void UpdateFade(float step) {
            ...
            if (m_sequence != null) {
                ...
                if (m_sequence.Clips.Length >= currentClipIdx || currentClipIdx < 0)
                {
                    curVolume = m_sequence.Volume;
                }
                else
