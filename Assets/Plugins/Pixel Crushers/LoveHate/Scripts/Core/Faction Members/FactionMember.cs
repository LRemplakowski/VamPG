...
namespace PixelCrushers.LoveHate
{
    ...
    public class FactionMember : MonoBehaviour
    {
        ...
        /// </summary>
        public string factionName { get { return (faction != null) ? faction.name : string.Empty; } }
        ...
        private Dictionary<Guid, float> m_noRepeatEndTime = new Dictionary<Guid, float>();
        ...
        public virtual void SwitchFaction(int newFactionID)
        {
            Unregister();
        ...
        {
            pad.pleasure = pad.arousal = pad.dominance = pad.happiness = 0;
            longTermMemory.ForEach(rumor => rumor.longTermExpiration = 0);
            CleanLongTermMemory();
            Register();
        }
        ...
        public virtual void StartMemoryCleanup()
        {
            StopMemoryCleanup();
            StartCoroutine(MemoryCleanupCoroutine());
        }

        public virtual void StopMemoryCleanup()
        {
            StopAllCoroutines();
        }
        ...
        public virtual void ForgetDeed(Guid deedGuid)
        {
        ...
        public virtual void ForgetDeed(int actorFactionID, int targetFactionID, string deedTag)
        {
        ...
        /// <param name="deedGuid">Deed GUID.</param>
        public virtual bool KnowsAboutDeed(Guid deedGuid)
        {
        ...
        /// <param name="targetFactionID">Target faction ID.</param>
        /// <param name="deedTag">Deed tag.</param>
        public virtual bool KnowsAboutDeed(int actorFactionID, int targetFactionID, string deedTag)
        {
        ...
        public virtual bool FindOldRumor(Rumor newRumor, out Rumor oldRumor)
        {
            for (int i = 0; i < longTermMemory.Count; i++)
            {
                var rumor = longTermMemory[i];
        ...
        public virtual void AddRumorToMemory(Rumor rumor)
        {
        ...
        public virtual void AddRumorToMemory(Rumor rumor, List<Rumor> memory)
        {
        ...
        public virtual void ModifyPAD(float happinessChange, float pleasureChange, float arousalChange, float dominanceChange)
        {
        ...
        public virtual void SetPersonalAffinity(int subjectFactionID, float affinity)
        {
        ...
        /// <param name="affinity">Affinity.</param>
        public virtual void SetPersonalAffinity(string subjectFactionName, float affinity)
        {
            if (!FindResources()) return;
        ...
        public virtual void ModifyPersonalAffinity(int subjectFactionID, float affinityChange)
        {
            if (!FindResources()) return;
        ...
        /// <param name="subjectFactionName">Subject faction name.</param>
        /// <param name="affinityChange">Affinity change.</param>
        public virtual void ModifyPersonalAffinity(string subjectFactionName, float affinityChange)
        {
            if (!FindResources()) return;
        ...
        /// <param name="subjectFactionID">Subject faction ID.</param>
        /// <param name="affinity">Affinity.</param>
        public virtual bool FindPersonalAffinity(int subjectFactionID, out float affinity)
        {
        ...
        /// <param name="subjectFactionID">Subject faction ID.</param>
        /// <param name="affinity">Affinity.</param>
        public virtual bool FindAffinity(int subjectFactionID, out float affinity)
        {
        ...
        /// <param name="subjectFactionID">Subject faction ID.</param>
        public virtual float GetAffinity(int subjectFactionID)
        {
        ...
        public virtual float GetAffinity(FactionMember subject)
        {
        ...
        /// <returns>The affinity.</returns>
        /// <param name="subjectFactionName">Subject faction name.</param>
        public virtual float GetAffinity(string subjectFactionName)
        {
        ...
        /// <param name="minAffinityEffect">Evaluating this deed will never cause faction's affinity to go below this value.</param>
        /// <param name="maxAffinityEffect">Evaluating this deed will never cause faction's affinity to go below above value.</param>
        /// <param name="noRepeatDelay">Faction members will ignore deeds that are repeated within this time frame.</param>
        public virtual void WitnessDeed(Deed deed, FactionMember actor, bool requiresSight, Dimension dimension = Dimension.Is3D)
        {
            if (!FindResources()) return;
            if (IsDeedInNoRepeatDuration(deed)) return;
            if (!requiresSight || CanSee(actor, dimension))
            {
                ...
                // If deed has a no-repeat time frame, record the time when it can be repeated:
                if (deed.noRepeatDuration > 0 && deed.guid != null)
                {
                    m_noRepeatEndTime[deed.guid] = Time.time + deed.noRepeatDuration;
                }
            }
        }
        ...
        /// </summary>
        protected virtual bool IsDeedInNoRepeatDuration(Deed deed)
        {
            if (deed == null || deed.noRepeatDuration == 0 || deed.guid == null) return false;
            float endTime;
            if (m_noRepeatEndTime.TryGetValue(deed.guid, out endTime))
            {
        ...
        public virtual void ShareRumors(FactionMember other)
        {
            if (!FindResources()) return;
            if (factionManager.debug)
            {
        ...
        /// <summary>
        /// Returns a value [0,+1] that indicates how well the specifies traits align with
        ...
        /// <returns>A value in the range 0 (no alignment) to +1 (100% alignment) indicating how well the values align.</returns>
        ...
        public Rumor DefaultEvaluateRumor(Rumor rumor, FactionMember source)
        {
            ...
            // traitImpactNorm[0,+1]: Modifies the opinion by how well the deed's traits align with our own:
            var traitAlignment = GetTraitAlignment(rumor.traits); //[0,+1]
            ...
            // Clamp affinity change by min/maxAffinityEffect:
            ...
            if (oldAffinityToActor >= rumor.minAffinityEffect && newAffinityToActor < rumor.minAffinityEffect)
            {
                newAffinityToActor = rumor.minAffinityEffect;
                changeInAffinityToActor = newAffinityToActor - oldAffinityToActor;
            }
            else if (oldAffinityToActor <= rumor.maxAffinityEffect && newAffinityToActor > rumor.maxAffinityEffect)
            {
                newAffinityToActor = rumor.maxAffinityEffect;
                changeInAffinityToActor = newAffinityToActor - oldAffinityToActor;
            }
        ...
        public virtual void ModifyPersonality(float[] traits, float multiplier)
        {
        ...
        public virtual void DeserializeFromString(string s)
        {
            if (string.IsNullOrEmpty(s) || factionManager == null || factionManager.factionDatabase == null || 
                factionManager.factionDatabase.personalityTraitDefinitions == null) return;
