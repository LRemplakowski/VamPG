...
//        }
        ...
        /// <summary>
        /// Checks to see if the index range of the active lighting profile isn't out of the index bounds of the lighting profiles
        /// </summary>
        /// <returns></returns>
        ...
        /// <summary>
        /// Checks to see if the index range of the active water profile isn't out of the index bounds of the water profiles
        /// </summary>
        /// <returns></returns>
        public static bool CheckWaterProfileIndexRange()
        {
            if (CheckIfSceneProfileExists())
            {
                if (GaiaGlobal.Instance.SceneProfile.m_selectedWaterProfileValuesIndex <= GaiaGlobal.Instance.SceneProfile.m_waterProfiles.Count - 1)
                {
                    return true;
                }
                else
        ...
        public static GaiaConstants.EnvironmentRenderer GetActivePipeline()
        {
            ...
            {
                return GaiaConstants.EnvironmentRenderer.Universal;
            }
        }
        ...
        /// <summary>
        /// Sets the scene view camera of the last active scene view to a given position and then make it look at the target position
        /// </summary>
        ...
        public static void SetSceneViewCam(Vector3 cameraPosition, Vector3 lookAtTarget)
        {
            ...
            sceneViewCam.transform.position = cameraPosition; 
            sceneViewCam.transform.rotation = Quaternion.LookRotation(lookAtTarget - cameraPosition);
            SceneView.lastActiveSceneView.AlignViewToObject(sceneViewCam.transform);
            float distance = Vector3.Distance(lookAtTarget, cameraPosition);
            //Workaround: The clipping planes can be set up weirdly after calling AlignViewToObject, making large objects invisible
            SceneView.lastActiveSceneView.Frame(new Bounds(lookAtTarget, new Vector3(distance, distance, distance)));
#endif
        }
        ...
        public static PlanarReflectionProbe GetHDRPPlanarReflectionProbe()
        {
            ...
            GaiaPlanarReflectionsHDRP hdrpPlanar = planarReflectionObject.GetComponent<GaiaPlanarReflectionsHDRP>();
            if (hdrpPlanar == null)
            {
                hdrpPlanar = planarReflectionObject.AddComponent<GaiaPlanarReflectionsHDRP>();
            }
        ...
        public static void CompressToMultiChannelFileImage(float[,,] input, string imageName, TextureFormat imageStorageFormat = Gaia.GaiaConstants.defaultTextureFormat, bool flip = false, bool exportPNG = true, bool exportJPG = true)
        {
            ...
            for (int image = 0; image < images; image++)
            {
                ...
                if (exportJPG)
                {
                    ...
                    PWCommon5.Utils.WriteAllBytes(imageName + image + ".jpg", jpgBytes);
                }
                ...
                if (exportPNG)
                {
                    ...
                    PWCommon5.Utils.WriteAllBytes(imageName + image + ".png", pngBytes);
                }
        ...
        public static void CompressToMultiChannelFileImage(string imageName, HeightMap r, HeightMap g, HeightMap b, HeightMap a, TextureFormat imageStorageFormat, GaiaConstants.ImageFileType imageFileType, float baseLevel, GaiaConstants.GaiaProWaterReflectionsQuality textureResolution = GaiaProWaterReflectionsQuality.Resolution1024)
        {
            ...
            switch (imageFileType)
            {
                case GaiaConstants.ImageFileType.Jpg:
                    ...
                    PWCommon5.Utils.WriteAllBytes(imageName + ".jpg", jpgBytes);
                    break;
        ...
        public static float[,] DecompressFromSingleChannelFileImage(string fileName, int width, int height, TextureFormat imageStorageFormat = Gaia.GaiaConstants.defaultTextureFormat, bool channelR = true, bool channelG = false, bool channelB = false, bool channelA = false)
        {
            ...
            {
                byte[] bytes = PWCommon5.Utils.ReadAllBytes(fileName);
        ...
        public static void ExportJPG(string fileName, Texture2D texture)
        {
            ...
            PWCommon5.Utils.WriteAllBytes(fileName + ".jpg", bytes);
        }
        ...
        public static void ExportPNG(string fileName, Texture2D texture)
        {
            ...
            PWCommon5.Utils.WriteAllBytes(fileName + ".png", bytes);
        }
        ...
        /// <summary>
        /// Picks a random element of a list of items with a weight (e.g. 20% for entry A, 60% for entry B, etc.)
        /// </summary>
        /// <typeparam name="T">The type of item to select / return</typeparam>
        /// <param name="entries">A list of item - weight pairs</param>
        /// <param name="randomizer">Randomizer for the random picking. Needs to be passed in from outside to prevent creation of the same values in a row.</param>
        /// <returns></returns>
        public static T PickRandomListElementWeightedChance<T>(IList<(T Item, int Chance)> entries, XorshiftPlus randomizer)
        {
            ...
            int offset = 0;
            (T Item, int RangeTo)[] rangedEntries = entries
                .OrderBy(item => item.Chance)
                .Select(entry => (entry.Item, RangeTo: offset += entry.Chance))
                .ToArray();
            
            int random = randomizer.Next(0,entries.Sum(item => item.Chance));
            return rangedEntries.First(item => random <= item.RangeTo).Item;
        }
        ...
        public static bool IsStampOperation(GaiaConstants.FeatureOperation operation)
        {
            return operation == GaiaConstants.FeatureOperation.RaiseHeight ||
                   ...
                   operation == GaiaConstants.FeatureOperation.MixHeight ||
        ...
        /// </summary>
        /// <param name="create">Creates the Terrain Loader Manager Object if it does not exist (default: true)</param>
        /// <returns></returns>
        ...
        public static GameObject GetPlayerGameObject()
        {
            ...
            playerObject = GameObject.Find("Gaia Car");
            if (playerObject != null)
            {
                return playerObject;
            }
        ...
        public static MonoBehaviour GetPlayerControllerSystem()
        {
            ...
            switch (player.name)
            {
                case "FPSController":
                {
                    ...
                    if (controller == null)
                    {
                        controller = player.GetComponentInChildren<FirstPersonController>();
                    }
                ...
                {
                    controller = player.GetComponent<ThirdPersonCharacter>();
                    if (controller == null)
                    {
                        controller = player.GetComponentInChildren<ThirdPersonCharacter>();
                    }
                ...
                case "Gaia Car":
                {
                    ...
                    if (controller == null)
                    {
                        controller = player.GetComponentInChildren<CarController>();
                    }
        ...
        /// <summary>
        /// Fixes color grading color filter if it's black
        /// </summary>
        ...
        public static void CheckPostFXV2ColorFilter(GaiaLightingProfileValues profile, Color checkColor)
        {
            try
            {
                ...
                {
                    if (profile.PostProcessProfileURP.TryGet(out UnityEngine.Rendering.Universal.ColorAdjustments colorGrading))
                    {
                        if (GaiaUtils.CheckColorKey(colorGrading.colorFilter.value, checkColor))
                        {
                            colorGrading.colorFilter.value = Color.white;
...
#if FLORA_PRESENT
        /// <summary>
        /// Select or create the Global Detail Manager for the detailer system
        /// </summary>
        public static GameObject GetOrCreateDetailGlobalManager(bool createManager = true)
        {
            GameObject detailManager = GameObject.Find(GaiaConstants.floraGlobalManagerObject);
            if (detailManager == null && createManager == true)
            {
                GameObject gaiaObj = GaiaUtils.GetRuntimeSceneObject();
                detailManager = new GameObject(GaiaConstants.floraGlobalManagerObject);
                detailManager.transform.parent = gaiaObj.transform;
                detailManager.transform.position = Vector3.zero;
                detailManager.AddComponent<FloraGlobalManager>();
            }
        ...
        public static void GUITextWithShadow(Rect rect, string text, GUIStyle guiStyle, GUIStyle shadowStyle)
        {
            ...
            shadowRect.x += 1f;
            shadowRect.y += 1f;
            EditorGUI.LabelField(shadowRect, text, shadowStyle);
            EditorGUI.LabelField(rect, text, guiStyle);
#endif
        }
