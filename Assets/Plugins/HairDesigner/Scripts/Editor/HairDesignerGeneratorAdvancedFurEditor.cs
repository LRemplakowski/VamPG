...
namespace Kalagaan
{
    namespace HairDesignerExtension
    {
        ...
        [CustomEditor(typeof(HairDesignerGeneratorAdvancedFurBase), true)]
        public class HairDesignerGeneratorAdvancedFurBaseEditor : HairGeneratorEditor
        {
            public enum eTab
            {
                //INFO,
            ...
            */
            ...
            public override void OnInspectorGUI()
            {
                ...
                HairDesignerGeneratorAdvancedFurBase g = target as HairDesignerGeneratorAdvancedFurBase;
                ...
                if (m_tab == eTab.MATERIAL)
                {
                    ...
                    if (g.m_hd.m_mr != null)
                    {
                        while (g.m_materialStrandEnabled.Count < g.m_hd.m_mr.sharedMaterials.Length)
                            g.m_materialStrandEnabled.Add(true);
                        while (g.m_materialShellEnabled.Count < g.m_hd.m_mr.sharedMaterials.Length)
                            g.m_materialShellEnabled.Add(true);
                    }

                    if (g.m_hd.m_smr != null)
                    {
                        while (g.m_materialStrandEnabled.Count < g.m_hd.m_smr.sharedMaterials.Length)
                            g.m_materialStrandEnabled.Add(true);
                        while (g.m_materialShellEnabled.Count < g.m_hd.m_smr.sharedMaterials.Length)
                            g.m_materialShellEnabled.Add(true);
                    }
                    ...
                    
                    {
                        ...
                        for (int i = 0; i < g.m_materialStrandEnabled.Count; ++i)
                        {
                            GUILayout.BeginHorizontal();
                            GUILayout.Label("Material " + (i+1));
                            g.m_materialStrandEnabled[i] = EditorGUILayout.Toggle( g.m_materialStrandEnabled[i], GUILayout.MaxWidth(20));
                            GUILayout.Label("Strands ");
                            g.m_materialShellEnabled[i] = EditorGUILayout.Toggle( g.m_materialShellEnabled[i], GUILayout.MaxWidth(20));
                            GUILayout.Label("Shells");
                    ...
                    GUI.enabled = true;
                    GUILayout.Space(10);
                    GUILayout.Label(new GUIContent("Shadows", (HairDesignerEditor.Icon(HairDesignerEditor.eIcons.BULLET))), EditorStyles.boldLabel);
                    g.m_strandsShadowCastingMode = (UnityEngine.Rendering.ShadowCastingMode)EditorGUILayout.EnumPopup("Strands shadow mode", g.m_strandsShadowCastingMode);
                    g.m_shellsShadowCastingMode = ( UnityEngine.Rendering.ShadowCastingMode)EditorGUILayout.EnumPopup("Shells shadow mode", g.m_shellsShadowCastingMode);
                    ...
                    if (g.m_hairMeshMaterial != null)
                    {
                        ...
                        //g.GetShaderParams().UpdateMaterialProperty(ref g.m_hairMeshMaterial, HairDesignerBase.eLayerType.FUR_SHELL);
                    }
                    if (GUI.changed)
                    {
                ...
                if (m_tab == eTab.MOTION)
                {
                    ...
                    if (g.m_motionData == null)
                        g.m_motionData = new HairDesignerGeneratorAdvancedFurBase.CSMotionData();

                    g.m_motionData.enable = EditorGUILayout.Toggle("Enable", g.m_motionData.enable);

                    //g.m_cs = EditorGUILayout.ObjectField("CS", g.m_cs, typeof(ComputeShader), true) as ComputeShader;
                    g.m_motionData.damping = EditorGUILayout.FloatField("Damping", g.m_motionData.damping);
                    g.m_motionData.bouncing= EditorGUILayout.FloatField("Bouncing", g.m_motionData.bouncing);

                    GUILayout.Box(" ", GUILayout.Height(50f), GUILayout.ExpandWidth(true));
                    HairDesignerPID pid = new HairDesignerPID(g.m_motionData.damping, g.m_motionData.bouncing, 0);
                    HairDesignerEditor.GUIDrawPidResponse(pid, GUILayoutUtility.GetLastRect(), 5f);

                    g.m_motionData.motionFactor = EditorGUILayout.FloatField("Motion factor", g.m_motionData.motionFactor);
                    g.m_motionData.timeScale = EditorGUILayout.FloatField("Time scale", g.m_motionData.timeScale);
                    g.m_motionData.centrigugalFactor = EditorGUILayout.FloatField("Centrifugal factor", g.m_motionData.centrigugalFactor);
                    g.m_motionData.gravity = EditorGUILayout.FloatField("Gravity", g.m_motionData.gravity);

                    g.m_motionData.windMain = EditorGUILayout.FloatField("Wind factor", g.m_motionData.windMain);
                    g.m_motionData.windTurbulence = EditorGUILayout.FloatField("Wind turbulence", g.m_motionData.windTurbulence);

                    g.m_motionData.random = EditorGUILayout.FloatField("Random", g.m_motionData.random);
                    g.m_motionData.interactionOffsetFactor = EditorGUILayout.FloatField("Collider offset factor", g.m_motionData.interactionOffsetFactor);




                    //g.m_rtex = EditorGUILayout.ObjectField(g.m_rtex, typeof(RenderTexture)) as RenderTexture;
                    EditorGUILayoutListField(g, "m_sphereColliders");
            ...
            bool TextureReady( HairDesignerShader s, ePaintingTool tool )
            {
                ...
                {
                    if (s != null && s.GetTexture((int)slot) != null)
                    {
                        try
                        {
                            s.GetTexture((int)slot).GetPixel(0, 0);
                            ready = true;
                        }
                        catch
                        {
                ...
                switch (m_currentTool)
                {
                    ...
                    case ePaintingTool.BRUSH:
                        if(m_secondaryTool == ePaintingTool.BRUSH)
                            ...
                            EditorGUILayout.HelpBox("Ctrl  : decrease height\nShift : smooth height", MessageType.Info);
                        break;
            ...
            {
                HairDesignerGeneratorAdvancedFurBase cp = copy as HairDesignerGeneratorAdvancedFurBase;
                HairDesignerGeneratorAdvancedFurBase g = target as HairDesignerGeneratorAdvancedFurBase;
                g.Init();                
                cp.Init();        
            }
            ...
            public override void PaintToolAction()
            {
                ...
                if (Event.current.type == EventType.MouseDrag)
                {
                    ...
                    if (m_tangents == null || m_tangents.Length != m_normals.Length)
                    {
                        m_tangents = mesh.tangents;
                        if(m_tangents.Length == 0)
                        {
                            Debug.LogError("This model doesn't have tangents. Please please set the option 'calculate tangent space' in the import settings of the model.");
                        }
                    }
                    if (m_triangles == null)
                                ...
                                if (m_secondaryTool == ePaintingTool.BRUSH)
                                {
                                    ...
                                    if (!m_CtrlMode && !m_ShiftMode)
                                    {
                                        ...
                                        if (vertexId != -1)
                                        {
                                            ...
                                            {
                                                normal += normals[ids[vid]].normalized;      //smooth normal                                      
                                            }
                                            normal.Normalize();
                                            */
                                    ...
                                    else if (m_CtrlMode && !m_ShiftMode)
                                    {
                                        ...
                                        for (int vid = 0; vid < ids.Count; ++vid)
                                        {
                                            ...
                                            c = brushTex.GetPixel(cx, cy);
                                            c = Color.Lerp(c, new Color(1f, .5f, .5f, c.a), m_brushIntensity * falloff*.1f);
